...
Each step is a stage in your pipeline (Module 07). Each environment has its own credentials, target hosts, and validation.
...
### `input` step (covered in Module 08 in depth)
```groovy
stage('deploy prod') {
  steps {
...
The pipeline pauses until someone in the `release-managers` group clicks Proceed (or Abort). The build holds an executor while waiting — for long pauses, see Module 08 on releasing the executor.
...
## What's Next
You've finished Section B. Section C is **everything-as-code** — managing the controller and its jobs from Git. **Module 10** starts with Shared Libraries.
